...
- **The repository has a licence.** It went public on the 17th without one, which makes public code all rights reserved while the instructions were telling people to fork it. `LICENSE` is Apache-2.0 for the code; the README puts the three reference-architecture drawings in `web-demo/public/ra/` under CC BY 4.0, credited to the working group that produced each, and says the member logos are not licensed for reuse. [PR #7]
- **Entering is for Summit sponsors.** The instructions now say it in their first section, before the benefits: a video on the ballot, a presentation slot, a stop on the Challenge Walk and Best in Show are part of the AI Networking Summit and open to its sponsors, Collaborative members included. A member not sponsoring this Summit keeps the head start, its working-group seat and its mark in the Collaborative Center, and the repository is open to anyone under its licence. [Nick, 18 Sep; PR #7]
- **The Summit is two days, 28 and 29 October.** Voting closes early in the afternoon of day two (exact time to be confirmed, ▲) and Best in Show is announced at the close of that day. [PR #7]
- **`docs/vendor-instructions.md` is the only copy anyone outside ONUG is given.** Its status line used to link a Google Doc "shareable copy" that only ONUG staff could open. On 17 September that Doc link went to every member in Slack; on the 18th the correction pointed members at this file, whose first paragraph sent them straight back to the same Doc. Members asked for access both times. The Doc link is gone from the file and from the Slack post, and both were checked signed out from the reader's side. The staff Doc still exists for marking up and is never linked from anything public. [PR #8]
- **What that taught us**, now in CLAUDE.md: before giving anyone a link, open it signed out and then read the page it lands on and follow what a reader would follow from there. A page that loads is not a page that works.

## After v0.3.3 — documents only (September 17, 2026, evening)

No change to the build. What moved around it:

- **Vendor instructions v4** (`docs/vendor-instructions.md`; at the time a Google Doc copy was linked from its status line, removed on the 18th, see above). v3 added the nine controls with the requirement ids each translates (Peter's three-lane challenge report), the 16 September team-meeting decisions (the challenge is open to every sponsor; four Best in Show categories by conference track, three of them the lanes; user's-choice voting in Whova closing at the end of lunch on day two; Challenge Walks over lunch on both days), and the members-first note (members had the demo, the repo and the instructions on 17 September, every sponsor on 28 September). v4 added joint rights (a worldwide, perpetual licence to ONUG to host, post, display and translate submissions), members' first priority for theatre presentation slots, and the note that the on-site schedule for walks and office hours may change and is posted in Slack. The call-for-entries email follows.
