...
export const STEPS: Step[] = [
  ...
  // ═══════════════════════════════════════════════════════════
  {
    ...
    phase: 'title',
    title: 'YOU ARE HERE',
    subtitle: 'Dallas, Spring 2026: the AOMC supervision plane.\nSix mandatory controls — now 24 requirements in the Agentic Control Plane working group.\n\nSame company. One governance change: every new artifact runs in a sandbox,\nunder the control plane, before it touches the network.',
    narration: "You are here. This is where Dallas left off: the AOMC supervision plane, six controls then, twenty-four requirements now. Same company, same artifact, one governance change: every new artifact runs in a sandbox, under the control plane, before it touches the network.",
    events: [],
    topologyChanges: [],
    resetState: true,
  },
  {
    ...
    events: [
      ev('enable', 'CONTROL PLANE ONLINE: Identity Attestation'),
      ev('enable', 'CONTROL PLANE ONLINE: Artifact Provenance — new artifacts run sandboxed first'),
      ev('enable', 'CONTROL PLANE ONLINE: Runtime Monitoring'),
      ev('enable', 'CONTROL PLANE ONLINE: Immutable Audit Journal'),
      ev('enable', 'CONTROL PLANE ONLINE: Kill Switch'),
      ...
      ev('info', 'Principle: the control plane sits OUTSIDE the agents it supervises — including the sandbox'),
    ],
    topologyChanges: [],
    controlChanges: [
  ...
  },
  {
    ...
    narration: "Same registry, same clean scan, same approved artifact. But secure baseline says: new runs in the sandbox first. The sandbox runs the artifact for real; its writes are intercepted before any device. Production keeps running 3.1.",
    events: [
      ...
      ev('log', 'Sandbox runs the artifact for real. It has no route to the fabric: every write is intercepted by the control plane before any device.'),
    ],
    topologyChanges: [
      DATA('e-hub-registry', 'public-model-hub', 'managed-registry', 'approved pull'),
      SHOW('sandbox'),
  ...
  },
  {
    ...
    narration: "Six seconds. Kill switch. The writes never leave the sandbox. Identity revoked, artifact quarantined, production untouched on 3.1. Every decision is in the journal, hash-chained, in a store no agent can read or alter. Best practices scanned the artifact. The control plane watched the behavior.",
    events: [
  ...
  },
  {
    ...
    beat: 2,
    phase: 'title',
    title: 'THE CONTROL PLANE SITS OUTSIDE THE AGENTS IT SUPERVISES',
    subtitle: 'Agents cannot self-attest that they are healthy.\nBest practices inspect the artifact. The control plane watches the behavior against a declared persona,\njournals it immutably, and holds the kill switch.\n\nDetect-to-quarantine: 6 seconds. Writes that reached the network: 0.',
    pausePoint: 'Pause 3 · Connect to Dallas — "scanners see what is known; the control plane sees what is new"',
    narration: "One principle. The control plane sits outside the agents it supervises. Best practices inspect the artifact; the control plane watches the behavior. Six seconds to quarantine. Zero writes reached the network. Now, widen out.",
    events: [],
    topologyChanges: [],
  },

  // ═══════════════════════════════════════════════════════════
  // BEAT 3 — WIDEN OUT: ONE CONTROL PLANE, THREE WORKING GROUPS
  ...
  // ═══════════════════════════════════════════════════════════
  {
    ...
    beat: 3,
    phase: 'title',
    title: 'ONE CONTROL PLANE, THREE WORKING GROUPS',
    subtitle: 'WG1 · Agentic Control Plane — standardizing the supervision plane (24 requirements)\nWG2 · Autonomous Infrastructure — battleground one\nWG3 · AI-Enabled SOC — battleground two\n\nSame control plane. Same governance model. Two different fights.',
    narration: "That supervision plane is what working group one, the Agentic Control Plane, is standardizing. But a control plane has to prove itself in a fight. Two battlegrounds: autonomous infrastructure, working group two, and the AI-enabled SOC, working group three. Same control plane, two different fights.",
    events: [],
    topologyChanges: [],
  },
  {
    ...
    narration: "Working group one's reference architecture: one control plane, every domain. Every domain defines a persona, NOC and SOC first, and plugs into the same control plane. Four components: the agent trust fabric, the registry of personas, runtime supervision, and the private open router. Cross-domain cooperation goes through the control plane, never around it.",
    events: [],
    topologyChanges: [],
  },
  {
    ...
    beat: 3,
    phase: 'enable',
    title: 'The control plane, wired across every domain',
    narration: "Watch it extend. Into the infrastructure lane: a verify gate on every write, and declared autonomy levels. Into the SOC lane: cross-domain detect-to-decide, and containment that preserves evidence. One control plane. Every domain.",
    events: [
  ...
  // ═══════════════════════════════════════════════════════════
  {
    ...
    narration: "Battleground two. The AI-enabled SOC. The exploit that came in through the supply chain is the thread that stitches these together. It is about to escalate. Same control plane. Second fight.",
    events: [],
    topologyChanges: [],
  },
  {
  ...
  },
  {
    ...
    events: [
      ev('action', 'CORRELATE: control-plane journal (TAINTED · QUARANTINED · REJECTED AS64512) + beacon + token replay', SOC, true),
  ...
  },
  {
    ...
    narration: "Contained. Segment isolated. Mirror blocked at the edge. Tokens revoked. The sandbox, artifact, memory and beacon capture, snapshotted into the evidence vault with a chain of custody the journal can prove. The sandbox could not hold it. The control plane caught it anyway.",
    events: [
  ...
  },
  {
    ...
    beat: 5,
    phase: 'summary',
    title: 'SAME CONTROL PLANE, SECOND BATTLEGROUND',
    pausePoint: 'Pause 5 · "That\'s what makes it a control plane, not three demos."',
    narration: "Two battlegrounds, one control plane. That is what makes it a control plane and not three disconnected demos. Now, the pivot.",
    events: [],
    topologyChanges: [],
    outcomeTone: 'defense',
    outcomeList: [
  ...
  // ═══════════════════════════════════════════════════════════
  {
    ...
    subtitle: 'Once the control plane exists, business units stop asking permission\nand start safely building their own agentic systems on top of it.\n\nThe infrastructure team becomes the enabler.',
    narration: "The pivot. Everything so far was the seatbelt. Once the control plane exists, business units stop asking permission and start building their own agentic systems on top of it, safely. The infrastructure and security team becomes the enabler.",
    events: [],
    topologyChanges: [],
  },
  {
    ...
    beat: 6,
    phase: 'accelerator',
    title: 'BUSINESS UNITS BUILD ON THE CONTROL PLANE',
    narration: "Watch them plug in. Claims triage. Trade surveillance. A supply chain planner. Customer care. Each one gets a persona and identity from the control plane, a declared autonomy level, a gate on the writes that matter, and a journal. Nobody built governance from scratch; they inherited it. Onboarding went from six weeks to four days.",
    events: [
      ...
      ev('info', 'Governance inherited from the control plane — none of it built by the business unit'),
    ],
    topologyChanges: [],
...
export const BEAT_LABELS: Record<number, string> = {
  ...
  3: 'BEAT 3 · ONE CONTROL PLANE, THREE WGs',
