...
Supporting docs: `README.md` (run it), `docs/handover.md` (state of the project, edit loop, open items), `docs/presenter-guide.md` (run of show), `docs/release-notes.md`, `docs/review-feedback.md` (what each reviewer said and what the demo did; the credit ledger), `docs/vendor-instructions.md` (the challenge, source of record; ONUG keeps a staff-only Google Doc copy for marking up, and **it must never be linked from this public repo** — every outside reader who followed that link on 17–18 September hit a request-access screen), `docs/vendor-call-email.md`, `docs/vendor-guide.md` (older, superseded by the instructions), `docs/narration-script.md` (generated, do not hand-edit).
...
- **Before giving anyone a link, check the reader's whole path, signed out.** Fetch it with no cookies, then read the page it lands on and follow every link a reader would follow from there. On 17–18 September members were sent to a staff-only document twice: once directly, and once through this repo's instructions page, which loaded fine and whose first paragraph pointed back at the same document. A 200 on the address is not the check. For a Drive file, read its permissions; for the portal, remember the demo is gated by `practitioners_first` and a technology member sees a holding page.
- **State what is there, not what was intended.** Before saying a draft is unsent, a post is unedited or a fix is live, look. A draft in Nick's mailbox can be sent the moment it exists, so build and test its links first and create it last.
- **Connector limits found the hard way.** The Gmail connector rewrites every link in a draft, including an explicit `href`, into a `google.com/url?q=` redirect that stops on a "Redirect Notice" page, and `update_draft` detaches a reply from its thread; when a clean link matters, give Nick the text to paste into his own mail. The Slack connector can post and reply but cannot edit a posted message; Nick edits it in Slack.
- **Who may enter comes before what they get.** Entering the challenge is for Summit sponsors, members included. Anything written for members or vendors states that condition first and names no company's sponsorship status; that is programme business and does not belong in this repo.
- The repo is licensed: Apache-2.0 for code, CC BY 4.0 for the drawings in `public/ra/`, member logos not licensed. A new kind of asset needs a line in the README's licence section.
